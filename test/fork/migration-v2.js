...
describe('V2 upgrade', function () {
  ...
  it('Migrate selected stakers to their own staking pools', async function () {
    ...
    console.log('Stakers data before migration:');
    stakerDataArray.map(data => console.log(data));
    ...
    for (const poolConfig of expectedPoolConfigurations) {
      ...
      expect(actualTrancheAmounts).to.be.deep.equal(expectedTrancheAmounts);
      ...
      for (const productAddress of productAddresses) {
        ...
        productBefore && console.log(`Checking product ${productId} - ${productAddress}`);
        ...
        productBefore && console.log({ productBefore, migratedProduct, actualWeight });
  ...
  it('purchase Cover at the expected prices from the migrated pools', async function () {
    ...
    console.log(`Buyer ${coverBuyer._address} buying cover for ${productId.toString()} on pool #1`);
